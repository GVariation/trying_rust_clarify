...
/*
    ...
    let text = String::from("bubblegum");
    greet(text);
    // println!("Второе обращение к переменной, text = {text}"); // не сработает
    
    let n = 200;
    display_number(n);
    println!("Второе обращение переменной, n = {n}");

    let s1 = create_string();
    // let s2 = create_string();
    // dbg!(s1, s2); // и после использовать s1 и s2 не получится
    ...
    println!("{text} содержит {length} символов");
}
